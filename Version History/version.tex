...
Sliders still do not control biamp rooms. This function was deemed unecessary as the only time these values should be adjusted 
is if we change the Biamp program. Sliders do work in rooms where there is only an Extron switcher controling the mix of 
program and speech
...
specific infomration which may be useful when reading through the code. This change was made to clean up and shorten files,
...
from a device like audio mutes or the proejctor on/off. Button events have been reduced with many functions being able to
be colapsed together. 
...
confidence monitor displaying the connected source, and a more robust shutdown fucntion. It fixed a long standing issue 
...
projection page used. The room also includes a Biamp which was experiemented with multiple control forms. This version uses
...
professors to wear the lappel mic in the room. Often times professors, who don't know there's a speech device in ther room, 
...
be incrememnted in their current version by 1 (for example KRS 3401 is on v1.4.0, so now is on 1.4.1)
